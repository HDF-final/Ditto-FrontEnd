...
test("navigation graph preserves all 147 selectable stores", async () => {
  ...
  assert.equal(storePlaces.length, 147);
  assert.equal(new Set(catalog.map((place) => place.navigation_key)).size, 147);
});

test("cross-floor course can use elevator-only and escalator-only routes", () => {
  const start = "B2_STORE_0031";
...
test("excluding both connector types blocks a cross-floor course", () => {
  assert.equal(
    findShortestPath(graph, "B2_STORE_0031", "6F_STORE_0033", {
...
test("itinerary keeps the dragged order and classifies floor transitions", () => {
  const stops = ["B2_STORE_0031", "1F_STORE_0031", "6F_STORE_0033"];
