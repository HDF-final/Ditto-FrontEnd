      ...
      try {
        ...
        if (news?.placeId) {
          ...
          if (directMatch && active) {
            setPlace(formatDbPlace(directMatch));
            return;
          }
        }
        ...
        for (const target of searchTargets) {
          ...
          for (const dbPlace of sortedDbPlaces) {
            ...
            if (normalizedTarget.includes(normalizedName)) {
              if (active) {
                setPlace(formatDbPlace(dbPlace));
              }
              return;
            }
          }
        ...
        <aside className="h-fit rounded-[16px] border border-line bg-white p-4 shadow-card transition-all lg:rounded-[28px] lg:p-7">
          <div className="flex items-center justify-between gap-2 border-b border-line pb-3 lg:gap-3 lg:pb-5">
            <div className="flex items-center gap-2 lg:gap-3">
              ...
              <h2 className="text-base font-black text-ink lg:text-2xl">{summaryTitle}</h2>
            </div>
              ...
              <div className="flex min-w-0 flex-1 flex-col items-start justify-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold text-white/60">
                    {place.category || "더현대 서울"} · {place.floor || "1F"}
                  </span>
                </div>
                ...
                <p className="mt-1 text-xs font-medium leading-snug text-white/75 break-keep sm:text-[13px]">
                  {`${place.floor ? `${place.floor} ` : ""}${place.name} · 실내 길찾기 지원`}
                </p>
...
function formatDbPlace(dbPlace) {
  ...
  const desc = `${floor} ${name} · 실내 길찾기 지원 매장`.trim();
  const image =
    dbPlace.imageUrl ||
    dbPlace.image_url ||
    dbPlace.image ||
  ...
  return {
    ...
    floor,
    floorCode: floor,
    category: dbPlace.category || "매장",
    description: desc,
    desc: dbPlace.description || desc,
    imageUrl: image,
    image,
    location: `더현대 서울 ${floor}`.trim(),
    modalMode: "standard",
    isNewsModal: true,
